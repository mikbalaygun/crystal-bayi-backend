...
router.get('/', authenticateToken, validateProductFilters, catchAsync(async (req, res) => {
  ...
  const [products, totalCount] = await Promise.all([
    Product.find(query)
      .select('stkno stokadi grupadi fiyat bakiye birim kdv uruntipi fgrp fagrp fatgrp')
      .skip(skip)
      .limit(limit)
      .lean(),
...
router.get('/search', authenticateToken, catchAsync(async (req, res) => {
  ...
  const projection = {
    ...
    stkno: 1, stokadi: 1, grupadi: 1, fiyat: 1, bakiye: 1, birim: 1, kdv: 1, uruntipi: 1
  };
...
router.get('/:stockNo', authenticateToken, catchAsync(async (req, res) => {
  ...
  const fromDb = await Product.findOne({ stkno: stockNo, isActive: { $ne: false } })
    .select({ stkno: 1, stokadi: 1, grupadi: 1, fiyat: 1, bakiye: 1, birim: 1, kdv: 1, uruntipi: 1, _id: 0 })
    .lean();
